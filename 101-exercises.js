...
let isPositiveEven = n => n > 0 && n % 2 === 0;
...
let isNegativeEven = n => n < 0 && n % 2 === 0;
...
let absoluteValue = n => Math.abs(n);
...
let isMultipleOfBothThreeAndFive = n => (n % 3 === 0 && n % 5 === 0);
...
let quotient = (n1, n2) => Math.floor(n1 / n2);
...
let sumOfSquares = (n1, n2) => square(n1) + square(n2);
...
let areaOfCircle = (radius) => square(radius) * Math.PI;
...
let circumference = radius => (radius * 2) * Math.PI;
...
let startsWithVowel = str => str.substring(0,1).split('').filter(isVowel).length > 0;
...
let endsWithVowel = str => str.substring(str.length - 1).split('').filter(isVowel).length > 0;
...
let startsAndEndsWithVowel = str => str.substring(0,1).split('').filter(isVowel).length > 0
    && str.substring(str.length - 1).split('').filter(isVowel).length > 0;
...
let secondToLast = input => input[input.length - 2];
...
let thirdToLast = input => input[input.length - 3];
...
let firstAndSecond = input => input.splice(0,2);
...
let firstAndLast = input => input.splice(0,1).concat(input.splice(input.length - 1, 1));
...
let firstToLast = input => input.concat(input.splice(0,1));
...
let hasEvens = input => input.filter(number => number % 2 === 0).length > 0;
...
let countEvens = input => input.filter(number => number % 2 === 0).length;
...
let hasOdds = input => input.filter(number => number % 2 !== 0).length > 0;
...
let countOdds = input => input.filter(number => number % 2 !== 0).length;
...
let countNegatives = input => input.filter(number => number < 0).length;
...
let countPositives = input => input.filter(number => number > 0).length;
...
let onlyPositiveEvens = input => input.filter(number => (number % 2 === 0 && number > 0));
...
let onlyPositiveOdds = input => input.filter(number => (number % 2 !== 0 && number > 0));
...
let onlyNegativeEvens = input => input.filter(number => (number % 2 === 0 && number < 0));
...
let onlyNegativeOdds = input => input.filter(number => (number % 2 !== 0 && number < 0));
...
let numberOfItemTypes = obj => obj.items.length;
...
let totalNumberOfItems = obj => obj.items.reduce((total,item) => {return total+item.quantity;}, 0);
...
let getAverageItemPrice = obj => obj.items.reduce((total,item) => {return total+item.price;},0)/numberOfItemTypes(obj);
...
let getAverageSpentPerItem = obj => obj.items.reduce((total, item) => {return total+(item.price * item.quantity);},0)/totalNumberOfItems(obj);
...
let mostSpentOnItem = obj => obj.items.sort((a, b) => (b.price * b.quantity) - (a.price * a.quantity))[0];
